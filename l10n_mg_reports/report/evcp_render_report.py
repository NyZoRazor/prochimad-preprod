...
class ParticularReport(models.AbstractModel):
    _name = 'report.l10n_mg_reports.evcp_report_template'
    ...
    def compute_years(self, date):
        nbr = []
        for year in range(2):
            new = int(date)  - year # if isinstance(date, unicode) else date - year
            nbr.append(new)
        return sorted(nbr)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if is_custom:
            ...
            if code == '1':
                ...
                domain_prec = [('date_maturity','>=',date_first-relativedelta(months=3)),('date_maturity','<=',date_end-relativedelta(months=3))]
                ...
                domain = [('date_maturity','>=',date_first),('date_maturity','<=',date_end)]
                domain_prec = [('date_maturity','>=',date_first-relativedelta(years=1)),('date_maturity','<=',date_end-relativedelta(years=1))]
                start_date_ef = date_first
                end_date_ef = date_end
        ...
        for year in list_years:
            ...
            result = {
                'result_prec_cap':0, 'result_prec_prime':0, 'result_prec_ecart':0, 'result_prec_result':0, 'result_prec_t':0,
                'change_meth_cap':0, 'change_meth_prime':0, 'change_meth_ecart':0, 'change_meth_result':0, 'change_meth_t':0,
                'correct_cap':0, 'correct_prime':0, 'correct_ecart':0, 'correct_result':0, 'correct_t':0,
                'prod_charge_cap':0, 'prod_charge_prime':0, 'prod_charge_ecart':0, 'prod_charge_result':0, 'prod_charge_t':0,
            ...
            for account in account_moves:
                ...
                result['result_prec_t'] = result['result_prec_cap'] + result['result_prec_prime'] + result['result_prec_ecart'] + result['result_prec_result']
                result['change_meth_t'] = result['change_meth_cap'] + result['change_meth_prime'] + result['change_meth_ecart'] + result['change_meth_result']
                result['correct_t'] = result['correct_cap'] + result['correct_prime'] + result['correct_ecart'] + result['correct_result']
                result['prod_charge_t'] = result['prod_charge_cap'] + result['prod_charge_prime'] + result['prod_charge_ecart'] + result['prod_charge_result']
                result['affect_t'] = result['affect_cap'] + result['affect_prime'] + result['affect_ecart'] + result['affect_result']
                result['operation_t'] = result['operation_cap'] + result['operation_prime'] + result['operation_ecart'] + result['operation_result']
                result['resultat_t'] = result['resultat_cap'] + result['resultat_prime'] + result['resultat_ecart'] + result['resultat_result']
                result['result_cap'] = result['result_prec_cap'] + result['change_meth_cap'] + result['correct_cap'] + result['prod_charge_cap'] + result['affect_cap'] + result['operation_cap'] + result['resultat_cap']
                result['result_prime'] = result['result_prec_prime'] + result['change_meth_prime'] + result['correct_prime'] + result['prod_charge_prime'] + result['affect_prime'] + result['operation_prime'] + result['resultat_prime']
                result['result_ecart'] = result['result_prec_ecart'] + result['change_meth_ecart'] + result['correct_ecart'] + result['prod_charge_ecart'] + result['affect_ecart'] + result['operation_ecart'] + result['resultat_ecart']
                result['result_result'] = result['result_prec_result'] + result['change_meth_result'] + result['correct_result'] + result['prod_charge_result'] + result['affect_result'] + result['operation_result'] + result['resultat_result']
                result['result_t'] = result['result_cap'] + result['result_prime'] + result['result_ecart'] + result['result_result']
