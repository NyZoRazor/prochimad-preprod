...
class CashFLowWizard(models.TransientModel):
    ...
    def confirm_cash_flow_view(self):
        data = {
        ...
        memo = self.env.ref('l10n_mg_reports.indirect_flux_tresorerie_report_html').report_action(self, data=data)
        return memo
        # memo['context']['report_action']['data']['start_date'] = fields.Date.from_string(
        #     memo['context']['report_action']['data']['start_date'])
        # memo['context']['report_action']['data']['end_date'] = fields.Date.from_string(
        #     memo['context']['report_action']['data']['end_date'])
        # return memo['context']['report_action']
    ...
    def print_pdf(self):
        ...
        # if self.type == 'direct':
        #     return self.env.ref('l10n_mg_reports.flux_tresorerie_report').report_action(self, data=data)
        # else:
        #     return self.env.ref('l10n_mg_reports.indirect_flux_tresorerie_report').report_action(self, data=data)
        return self.env.ref('l10n_mg_reports.indirect_flux_tresorerie_report').report_action(self, data=data)
    ...
    def export_excel(self):
        ...
        return {
            ...
            'data': {'model': 'cash.flow.wizard',
                     ...
                     'output_format': 'xlsx',
                     'report_name': _("Flux tresorerie direct") if self.type == 'direct' else _(
                         "Flux tresorerie indirect"),
                     },
            'report_type': 'xlsx',
        }
    ...
    def get_xlsx_report(self, data, response):
        if data['type'] == 'direct':
            ...
            worksheet.write(6, 1,
                            ...
                            bolding)
            worksheet.write(7, 0, _('Flux de tresorerie lie a des evenements extraordinaires (a preciser)'),
                            text_format)
            ...
            worksheet.write(8, 1,
                            '{:10,.2f}'.format(float(result['t_flux_operationnel'])).replace(',', ' ').replace('.',
                                                                                                               ','),
                            bolding)
            worksheet.merge_range('A10:B10', _('FLUX DE TRESORERIE LIEES AUX ACTIVITES D\'INVESTISSEMENT'),
                                  title_format)
