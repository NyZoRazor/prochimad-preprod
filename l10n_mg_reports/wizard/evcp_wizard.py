...
class EvcpReportViewWizard(models.TransientModel):
    ...
    period = fields.Selection([
        # ('1_1','Ce mois'),
        # ('2_1','Ce trimestre'),
        ('3_1', 'Cette année fiscale'),
        # ('1_2','Le mois dernier'),
        # ('2_2','Dernier trimestre'),
        ('3_2', 'Dernière année fiscale')
    ], default='3_1')
    is_custom = fields.Boolean(default=False)

    def confirm_evcp_view(self):
        data = {
        ...
        memo = self.env.ref('l10n_mg_reports.evcp_report').report_action(self, data=data)
        return memo
        # memo['context']['report_action']['data']['start_date'] = fields.Date.from_string(memo['context']['report_action']['data']['start_date'])
    ...
    def confirm_evcp(self):
        ...
        start_date = str(self.start_date).replace('-', '__')
        end_date = str(self.end_date).replace('-', '__')
        period = str(self.period)
        is_custom = str(self.is_custom)
        return {
            'type': 'ir.actions.act_url',
            'url': '/web/evcp_binary/download_evcp?p_start_date=%s&p_end_date=%s&p_period=%s&p_is_custom=%s&p_date_start=%s' % (
                start_date,
                end_date, period, is_custom, date_start),
            'target': 'self',
        }
    ...
    def print_xlsx(self):
        ...
        temp_data = self.env.ref('l10n_mg_reports.evcp_report').report_action(self, data=temp_data,
                                                                                                 config=False)
        data = {
    ...
    def get_xlsx_report(self, data, response):
        report = self.env['report.l10n_mg_reports.evcp_report_template']
